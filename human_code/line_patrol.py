...
import math
import numpy as np
import human_code.missions as missions
...
def Tracing(orgimage, r_w = resize_width, r_h = resize_height, r = roi, l_c = line_color, l_t = line_thickness):
    ...
    center_ = []
    max_area = 0
    cnts, _ = cv2.findContours(Imask , cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_TC89_L1)#找出所有轮廓
...
while True:
    ...
    if orgFrame is not None and ret:
        if Running:
            ...
            # missions.edge_detection(orgFrame, 160, 120)
            t2 = cv2.getTickCount()
