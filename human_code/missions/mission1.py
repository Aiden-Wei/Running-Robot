...
PWMServo.setServo(1, 2000, 500) #舵机，步长，时间
...
def get_image():
    ...
    while True:
        if Running:
            if cap.isOpened():
                ret, orgFrame = cap.read() #ret表示是否读入照片；orgFrame为照片信息；
            else:
                time.sleep(0.01)
        else:
            time.sleep(0.01)
...
def Tracing(orgimage, r_w = resize_width, r_h = resize_height, r = roi, l_c = line_color, l_t = line_thickness):
    ...
    h, s, v = cv2.split(orgframe)
    orgframe = cv2.cvtColor(orgframe, cv2.COLOR_BGR2HSV); #将图片转换到HSV空间
    v = cv2.equalizeHist(v)
    orgframe = cv2.merge((h,s,v))
    ...
    area = cv2.contourArea(cnt_large)
    if area > 1200:
        stop_run = True
    else:
        stop_run = False
